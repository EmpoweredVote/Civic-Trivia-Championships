...
The harness is throwaway and deliberately never merged — it adds a `/perf-lab` route that has
no business shipping. The `spike/bobit-perf` branch it lived on was deleted on 2026-09-10;
the code is preserved at the annotated tag **`archive/bobit-perf-spike`** (`0fbf867`):
...
```bash
git checkout archive/bobit-perf-spike     # detached; do not branch from it to ship
cd frontend && npm run build && npm start
node scripts/perf-bench.mjs
```

`perf-bench.mjs` drives `/perf-lab`, so it is inert without that route — take the whole tag,
not just the script.
