...
- **Question text** - full text, large font, dark text
- **Answer options** - numbered list (A, B, C, D), correct answer highlighted with green bg + checkmark icon. Other options in neutral styling.
- **Explanation** - below options, gray bg section with the explanation text
- **Learn More** - if source.url exists, render as external link (opens new tab) with source.name as label
...
cd frontend && npx tsc --noEmit && npm run build
```
Build compiles. Verify QuestionDetailPanel.tsx exists and imports Dialog from @headlessui/react. Verify QuestionsPage.tsx renders QuestionDetailPanel.
  </verify>
  <done>
Slide-over detail panel opens when clicking a question row, showing full question content (text, 4 options with correct highlighted, explanation, Learn More link), quality assessment (score + violations with severity badges), and telemetry data (encounters, correct count, difficulty rate). Prev/next arrows navigate between questions. Panel closes on backdrop click, X button, or Escape key.
  </done>
</task>

<task type="checkpoint:human-verify" gate="blocking">
  <what-built>Question explorer page at /admin/questions with sortable/filterable table, search, pagination, and slide-over detail panel with quality violations and telemetry display</what-built>
  <how-to-verify>
...
13. Verify: Panel shows quality score and any violations with severity badges
14. Verify: Panel shows encounter count, correct count, and calculated difficulty rate
15. Verify: Questions with fewer than 20 encounters show "Insufficient data" for difficulty rate
16. Verify: Prev/next arrows in panel header navigate between questions
17. Verify: Panel closes on X button, Escape key, or backdrop click
  </how-to-verify>
...
5. Difficulty rate displays correctly per TELE-03
6. URL state persistence works for all filters and sort
...
- Click question opens slide-over detail panel with full content, quality assessment, and telemetry (EXPL-02)
