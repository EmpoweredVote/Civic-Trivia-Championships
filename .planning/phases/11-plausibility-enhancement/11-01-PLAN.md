...
must_haves:
  truths:
    - "Fast wrong answers on Q1-Q9 are flagged based on difficulty-adjusted thresholds"
    - "Anonymous users are never flagged"
    - "Q10 wager question is never checked for plausibility"
    - "Fast correct answers are never flagged"
    - "Timer multiplier users get proportionally scaled thresholds"
    - "Penalties only apply after 3+ flagged answers in one game"
    - "Penalty zeroes speed bonus on flagged answers (forward-only)"
    - "Session plausibilityFlags counter resets each new game"
  artifacts:
...
2. In `backend/src/services/sessionService.ts`:
   ...
   - Update `QUESTION_DURATION` from 25 to 20 (Phase 10 changed frontend timer to 20s — backend must match for accurate responseTime calculation)
   - Update `MAX_PLAUSIBLE_TIME_REMAINING` from 25 to 20 (same reason)
  ...
  <done>
    ...
    - Duration constants match frontend (20s questions, 50s final)
  </done>
</task>
...
2. In `backend/src/services/sessionService.ts` `submitAnswer()` method, rewrite the plausibility check block:
   ...
   d. Skip flagging if `isCorrect` (fast correct answers are legitimate knowledge)
   e. If NOT correct and NOT exempt:
      ...
      - If `responseTime < threshold`: set `flagged = true`, increment `session.plausibilityFlags += 1`, console.warn with difficulty, multiplier, threshold details
  ...
  <verify>
    ...
    - Test with curl: Create session, submit a normal answer — returns score as before
    - Verify console.warn appears when submitting a suspiciously fast wrong answer (responseTime < threshold)
  ...
  <done>
    ...
    - Only fast WRONG answers are flagged
    - Anonymous users are never flagged
...
2. Server starts: `cd backend && npm run dev` (no runtime errors)
3. Manual test flow:
   ...
   - Verify flagged field set for suspiciously fast wrong answers (server logs)
...
- Penalty zeroes speed bonus (forward-only, no retroactive)
